...
function Calendar({
  ...
  const { root: userRootClassName, ...restUserClassNames } =
    userClassNames ?? {}
  ...
  return (
    <DayPicker
      ...
      classNames={{
        root: cn("w-fit", defaultClassNames.root, userRootClassName),
        months: cn(
        ...
        caption_label: cn(
          ...
          captionLayout === "label"
            ? "text-sm"
            : "cn-calendar-caption-label flex items-center gap-1 rounded-[var(--cell-radius)] text-sm [&>svg]:size-3.5 [&>svg]:text-muted-foreground",
          defaultClassNames.caption_label
        ),
        /** v9: `<table>` — avoid `flex` on `tr` (`weekdays` / `week`); it breaks column layout */
        month_grid: cn(
        ...
        weekday: cn(
          "h-[var(--cell-size)] w-[var(--cell-size)] rounded-[var(--cell-radius)] p-0 text-center align-middle text-[0.8rem] font-normal text-muted-foreground select-none",
        ...
        week_number: cn(
          "w-[var(--cell-size)] p-0 text-center align-middle text-[0.8rem] text-muted-foreground select-none",
          defaultClassNames.week_number
        ),
        day: cn(
          "group/day relative aspect-square h-[var(--cell-size)] w-[var(--cell-size)] rounded-[var(--cell-radius)] p-0 text-center align-middle select-none [&:last-child[data-selected=true]_button]:rounded-r-[var(--cell-radius)]",
          showWeekNumber
