...
프로젝트 의사결정 아이템 관리 프로토타입을 **React + Vite + Shadcn UI + React Hook Form + Zod + CSS Modules + Feature-Sliced Design(FSD)** 로 이전하기 위한 선행 문서 모음입니다.

## 목표 스택

| 영역 | 목표 |
|------|------|
| 런타임·빌드 | React 18+, Vite |
| UI 컴포넌트 | Shadcn UI (Radix 기반) |
| 폼 | React Hook Form |
| 스키마·검증 | Zod (`@hookform/resolvers/zod`) |
| 스타일 | CSS Modules (컴포넌트/슬라이스 단위) |
...
| [04-ui-and-styles.md](./04-ui-and-styles.md) | CSS Modules 분해, Shadcn 도입 순서 |
...
8. **접근성·폴리시:** Shadcn/Radix 패턴에 맞춘 키보드·포커스·라벨 정비.
...
| [04-ui-and-styles.md](./04-ui-and-styles.md) | `styles.css` → Modules/Shadcn 이전 **작업 지시서**. 스타일 이전이 끝나면 중복. |
...
| `seol-logo.png` 등 정적 자산 | 새 `public/` 또는 번들 경로로 이전한 뒤 기존 참조가 없을 때. |
